...
function startWorking(maxScore=12) {
	...
	let tasks = [];
	let prev = "";
	for(let i=0; i<=maxScore; i++) for(let j=0; j<=maxScore; j++) tasks.push(async () => {
		if(Object.keys(prestoredScorePaths).includes(`${i}-${j}`) || containsArray([i,j],deadEnds)) return;
		let finall = scoriperm(i,j);
		document.getElementById("currentDisplay").innerHTML = `${i}-${j}: ${finall.length} paths`;
		prestoredScorePaths[`${i}-${j}`] = finall;
	});
	Promise.all(tasks.map(e => e()));
	sendData();
}
